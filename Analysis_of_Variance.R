...
#Obtaining Data
CropData <-read.table("~/Data_Analysis/cropData.txt",header =TRUE,sep="\t", fill = TRUE)
head(CropData, 10)
...
# Summarize the ANOVA results with interaction
summary(fit)

fit1 <- aov(yield ~ factor(fertilizer) + factor(density), data = CropData)

# Summarize the ANOVA results without interaction
summary(fit1)
